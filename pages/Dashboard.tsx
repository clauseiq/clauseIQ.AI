...
import { analyzeContract, getQuickAnchor } from '../services/geminiService';
...
export const Dashboard: React.FC = () => {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        // Fire and forget - don't await, let UI update independently
  ...
  const handleViewHistoryItem = (item: any) => {
    // Navigate to result with the saved JSON
    navigate('/result', { state: { result: item.full_json, contractText: "Text not saved to history" } });
  };
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 animate-reveal">
                          ...
                          {isGettingAnchor ? (
                            ...
                            <>
                              <div className="flex items-center justify-center space-x-2 mb-3">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${quickAnchor.verdict === 'Risky' || quickAnchor.verdict === 'Dangerous' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                                  }`}>
                                  {quickAnchor.verdict}
                                </span>
                              </div>
        ...
        {isAuthenticated && history.length > 0 && (
          <div className="mt-16 animate-reveal">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Recent Analyses</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {history.map((item) => (
                <div key={item.id} onClick={() => handleViewHistoryItem(item)} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-blue-500 hover:shadow-lg transition-all cursor-pointer group">
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-xs font-bold text-slate-400 uppercase">{new Date(item.created_at).toLocaleDateString()}</span>
                    <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${item.risk_score >= 80 ? 'bg-emerald-100 text-emerald-700' :
                        item.risk_score >= 60 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'
                      }`}>Score: {item.risk_score}</span>
                  </div>
                  <h3 className="font-bold text-slate-900 dark:text-white mb-2 truncate">{item.contract_name || "Untitled Contract"}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-3 mb-4">{item.summary_text}</p>
